...
from core.config import DATA_DIR, RAG_DIR
...
def _load_armor() -> list[Document]:
    ...
    for aset_id, pieces in armor_grouped_by_set.items():
        ...
        # Enriquecimento de metadados para busca (Rank e Set)
        aset_data = armorset_lookup.get(aset_id, {})
        rank_val = aset_data.get("rank", "").upper()
        rank_tag = ""
        if rank_val == "MASTER":
            rank_tag = " [MASTER RANK / RM]"
        elif rank_val == "HIGH":
            rank_tag = " [HIGH RANK / ALTO]"

        lines = [f"=== CONJUNTO DE ARMADURA: {set_name}{rank_tag} ==="]
        ...
        for a in sorted_pieces:
            ...
            # Normalização de nomes de variações (Alfa/Beta) para melhor retrieval
            pname_normalized = pname
            if " α" in pname or " alpha" in pname.lower():
                pname_normalized += " (Alfa / Alpha / α+)"
            if " β" in pname or " beta" in pname.lower():
                pname_normalized += " (Beta / β+)"

            lines.append(f"\n  > PEÇA: {pname_normalized} ({a.get('armor_type', 'desconhecido')})")
...
# ============================================================
# SKILL REVERSE LOOKUP (QUEM DÁ CADA SKILL)
# ============================================================
def _load_skill_reverse_lookup() -> list[Document]:
    """
    Cria documentos de consulta reversa: para cada Skill, lista onde ela pode ser encontrada.
    Isso ajuda imensamente o RAG a responder "quais armaduras tem X".
    """
    docs = []
    
    # 1. Carregar Dados de Texto
    skilltree_text = _build_text_lookup("skilltree_text.xml")
    armor_text = _build_text_lookup("armor_text.xml")
    charm_text = _build_text_lookup("charm_text.xml")
    deco_text = _build_text_lookup("decoration_text.xml")
    
    # 2. Carregar Associações
    armor_skills = _parse_xml("armor_skill.xml")
    charm_skills = _parse_xml("charm_skill.xml")
    decos = _parse_xml("decoration.xml")
    
    # 3. Carregar info de armaduras para saber o Rank (Master/High/Low)
    armors = _parse_xml("armor.xml")
    armor_rank_map = {a.get("id"): a.get("rank", "Unknown") for a in armors}

    # Agrupar por SkillTree ID
    by_skill: dict[str, dict[str, list[str]]] = {}

    def add_source(st_id: str, source_type: str, text: str):
        if st_id not in by_skill:
            by_skill[st_id] = {}
        if source_type not in by_skill[st_id]:
            by_skill[st_id][source_type] = []
        by_skill[st_id][source_type].append(text)

    # Processar Armaduras
    for ash in armor_skills:
        stid = ash.get("skilltree_id", "")
        aid = ash.get("armor_id", "")
        lvl = ash.get("level", "1")
        # Normalizar rank para minúsculo para evitar erros de case-sensitivity
        rank = armor_rank_map.get(aid, "unknown").lower()
        name = _display_name(armor_text.get(aid, {}))
        if stid and name:
            # Injetar tags de rank diretamente no nome da fonte para maximizar recall
            rank_tag = ""
            if rank == "master": rank_tag = " [MASTER RANK / RM / MR]"
            elif rank == "high": rank_tag = " [HIGH RANK / ALTO / HR]"
            elif rank == "low": rank_tag = " [LOW RANK / BAIXO / LR]"
            add_source(stid, rank, f"{name}{rank_tag} (+{lvl})")

    # Processar Amuletos
    for csh in charm_skills:
        stid = csh.get("skilltree_id", "")
        cid = csh.get("charm_id", "")
        lvl = csh.get("level", "1")
        name = _display_name(charm_text.get(cid, {}))
        if stid and name:
            add_source(stid, "amuletos", f"{name} (+{lvl})")

    # Processar Decorações (Joias)
    for d in decos:
        stid = d.get("skilltree_id", "")
        did = d.get("id", "")
        slot = d.get("slot", "1")
        name = _display_name(deco_text.get(did, {}))
        if stid and name:
            add_source(stid, "joias", f"{name} (Slot {slot})")

    # Gerar Documentos
    for stid, storage in by_skill.items():
        st_info = skilltree_text.get(stid, {})
        sname = _display_name(st_info)
        if not sname: continue
        
        lines = [f"=== TABELA OFICIAL DE FONTES E ARMADURAS: {sname} ==="]
        lines.append(f"Este documento é a ÚNICA fonte da verdade sobre quais equipamentos dão {sname}.")
        lines.append(f"Se houver equipamentos listados abaixo para um Rank, ignore qualquer informação dizendo que não existem fontes para esse Rank.\n")
        
        # Ordem de rank para exibição com títulos redundantes para garantir match
        rank_mapping = {
            "master": "FONTES DE MASTER RANK (RM / MR / MASTER)",
            "high": "FONTES DE HIGH RANK (RANK ALTO / HR / ALTO)",
            "low": "FONTES DE LOW RANK (RANK BAIXO / LR / BAIXO)",
            "amuletos": "AMULETOS E CHARMS",
            "joias": "JOIAS E DECORAÇÕES (SLOTS)"
        }
        
        for raw_rank, title in rank_mapping.items():
            if raw_rank in storage:
                lines.append(f"--- {title} ---")
                unique_sources = sorted(list(set(storage[raw_rank])))
                for source in unique_sources:
                    lines.append(f"  • {source} dá a skill {sname}")
                lines.append("")

        docs.append(Document(
            text="\n".join(lines).strip(),
            metadata={"source": "skill_lookup", "skill_name": sname, "skilltree_id": stid},
        ))
...
def load_all_documents(progress_callback=None) -> list[Document]:
    ...
    loaders = [
        ...
        ("Carregando skills (Reverse Lookup)...", _load_skill_reverse_lookup),
        ("Carregando skills (Descrições)...", _load_skills),
